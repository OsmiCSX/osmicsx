...
/**
 * Detect using Switch Case for responsive width, height, text
 */
const detectResponsive = style => {
  switch(style[0]) {
    case "w":
      return {
        width: scaleWidth(style[1])
      }

    case "h":
      return {
        height: scaleHeight(style[1])
      }

    case "text":
      return {
        fontSize: scaleWidth(style[1])
      }
  }
}
